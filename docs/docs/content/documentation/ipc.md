...
fn main() -> rayforce::Result<()> {
    ...
    // A disconnect is not an error and does not interrupt the loop: the peer
    // simply stops resolving. Slice the loop and check between slices, or a
    // process whose connection was reaped sits there looking healthy while
    // receiving nothing.
    while sub.is_alive() {
...
!!! note "Teardown order"
    ...
    The loop itself belongs to the `Runtime` and is torn down with it, so a
    `Poll` outliving its runtime is not a hazard — it simply stops working, and
    every call on it returns an error.
